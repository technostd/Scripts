...
CheckUpdate(Version) 
{
	...
	MsgBox, % 4+64+256, Обновление, Есть новая версия, обновить с %GuiVersion% на %GuiNewVersion%?
	IfMsgBox, No
	{
		Return
	}
...
Gui 1:Add, Text, x002 y700 w100 h15, v%GuiVersion%
Gui 1:Add, Text, x598 yp w300 h15 Right, %TextFooter%

Gui 1:Font, s16 c000000
Gui 1:Add, Button, x025 y660 w150 h30 gButtonEdit, Изменить
...
Gui 1:Add, GroupBox, x015 y050 w290 h600, [ Основное ]
Gui 1:Add, GroupBox, x305 y050 w290 h220, [ Задержание ]
Gui 1:Add, GroupBox, x305 y255 w290 h395, [ Оружие ]
Gui 1:Add, GroupBox, x595 y050 w290 h600, [ Команды ]
...
Gui 1:Add, Text, xp yp+20 w100 h20, /скан
...
Gui 1:Add, Text, xp yp+20 w100 h20, /лишправ <ID срок>
Gui 1:Add, Text, xp yp+20 w100 h20, /штраф <ID сумма причина>
...
Gui 1:Add, Text, xp yp+20 w170 h20, КПК (достать, убрать)
Gui 1:Add, Text, xp yp+20 w170 h20, Документы (взять, вернуть)
Gui 1:Add, Text, xp yp+20 w170 h20, Пробить по базе (КПК)
Gui 1:Add, Text, xp yp+20 w170 h20, Составить протокол
Gui 1:Add, Text, xp yp+20 w170 h20, Отдать на подпись
Gui 1:Add, Text, xp yp+20 w170 h20, Дать копию протокола
Gui 1:Add, Text, xp yp+20 w170 h20, Метка о невозможности
Gui 1:Add, Text, xp yp+20 w170 h20, Метка об отказе
Gui 1:Add, Text, xp yp+20 w170 h20, Убрать копию
...
Gui 1:Show, h715 w900, ГИБДД #5 | AHK Province
Return
...
Gui 2:Add, Text, x220 yp+40 w300 r11, Звание | *Старшина*
...
*/
    ...
    :?:/лишправ::
    GetArgsForHotstring("госномер", "ID")
    SendInput {Enter}
    Gosub Takecarlic
    Return
  ...
  ; Текстовые бинды с метками
    ...
    DTPPhotofix:
    SendChat("do Фотоаппарат в кармане.", "500")
    ...
    DTPBrakeDist:
    SendChat("do Рулетка в кармане.", "500")
    SendChat("me достав рулетку, нача" lla " измерения тормозных путей", "5000")
    SendChat("do Измерение завершено.", "500")
    SendChat("me собрав рулетку, убра" lla " её в карман", "0")
    Return
    
    DTPProtocol:
    SendChat("do На поясе у инспектора висит сумка с бланками.", "500")
    SendChat("me доста" lla " бланк осмотра места ДТП, зарисова" lla " схему ДТП", "500")
    SendChat("me сдела" lla " на схеме пометки со слов граждан", "500")
    SendChat("me нанес" la " на схему значения, измеренные рулеткой, положи" lla " бланк в сумку", "500")
    ...
    Taumetr:
    SendChat("me включи" lla " тауметр", "500")
    SendChat("me нача" lla " измерять тонировку на лобовом стекле и передних боковых стеклах", "500")
    ...
    DVROn:
    SendChat("me откры" lla " нужный файл и подготови" lla " его к просмотру", "500")
